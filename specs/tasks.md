...
- **MVP 核心能力**：
  ...
  - Slash Commands（`.claude/commands/` 与 `~/.claude/commands/`）与基础 Hooks。
...
- [ ] **T010 CLI 入口（click）最小命令树（TDD）**
  - **依赖**：T003, T004
  - **产物**：`claude_code/cli/main.py`（或等价入口），`--help` 工作
...
- [ ] **T020 Settings 数据结构与默认值（TDD）**
  - **依赖**：T003
  - **产物**：`claude_code/config/settings.py`（按 plan.md 的字段：llm/permissions/hooks/default_model...）
  - **验证**：单测覆盖默认值与类型校验（Python 3.8 类型注解兼容）

- [ ] **T021 分层配置加载器（TDD）**
  - **依赖**：T020
  - **产物**：`claude_code/config/loader.py` 支持：用户全局、项目共享、本地覆盖、CLI 覆盖
  - **验证**：表格驱动测试覆盖优先级合并
...
- [ ] **T030 定义 `LLMClient` 抽象接口（TDD）**
  - **依赖**：T003
  - **产物**：`claude_code/llm/client.py`（chat_completion/get_model/supports_streaming/supports_tools）
  - **验证**：单测覆盖接口契约（例如最小 fake 实现）

- [ ] **T031 LLM Factory（TDD）**
  - **依赖**：T030, T021
  - **产物**：`claude_code/llm/factory.py` 根据配置选择实现，支持回退策略
  - **验证**：单测覆盖 provider 切换与回退

- [ ] **T032 OpenAIClient 实现（openai==0.28.1）（TDD）**
  - **依赖**：T030, T021
  - **产物**：`claude_code/llm/openai_client.py` 支持 `api_base`、基本错误处理、非流式调用
  - **验证**：单测（用 fake server 或注入方式）覆盖请求构造；不硬编码 key

- [ ] **T033 RequestsClient 备选实现（requests==2.28.2）[P]（TDD）**
  - **依赖**：T030, T021
  - **产物**：`claude_code/llm/requests_client.py`（OpenAI 兼容协议）
  - **验证**：单测覆盖超时/重试/证书配置分支
...
- [ ] **T040 `@`/`!` 解析器（TDD）**
  - **依赖**：T003
  - **产物**：`claude_code/interaction/parser.py`（识别 `@file`、`@dir`、`!command`）
  - **改动点**：`claude_code/interaction/parser.py`，`tests/test_parser.py`
...
- [ ] **T041 Gitignore 感知目录加载（TDD）**
  ...
  - **改动点**：`claude_code/core/context.py` 或 `claude_code/interaction/parser.py`（以最终职责归属为准），`tests/test_parser.py`
  - **验证**：`pytest -q tests/test_parser.py`
...
- [ ] **T042 上下文格式化（文件/目录树）（TDD）**
  ...
  - **改动点**：`claude_code/core/context.py`，`tests/test_context.py`
  - **验证**：`pytest -q tests/test_context.py`
  - **DoD**：输出格式至少包含：路径标识、内容边界（开始/结束），且对超长内容有可预测的裁剪策略（通过测试约束）

- [ ] **T043 命令执行器 `CommandExecutor`（TDD）**
  - **依赖**：T003
  - **产物**：`claude_code/core/executor.py`：Windows 7 PowerShell / Unix bash，捕获 stdout/stderr，UTF-8 处理
  - **改动点**：`claude_code/core/executor.py`，`tests/test_executor.py`
...
- [ ] **T044 命令输出注入下一轮上下文（TDD）**
  ...
  - **改动点**：`claude_code/core/agent.py`（或消息历史管理模块），`tests/test_agent.py`
...
- [ ] **T050 ContextManager：加载文件/目录（TDD）**
  - **依赖**：T042
  - **产物**：`claude_code/core/context.py`：`load_file`/`load_directory`
...
- [ ] **T052 `CLAUDE.md` 模块化导入（`@` 引用其他文件）（TDD）**
  ...
  - **改动点**：`claude_code/core/context.py`（或独立的 loader），`tests/test_context.py`
  - **验证**：`pytest -q tests/test_context.py`
...
- [ ] **T060 Agent 基础对话循环（TDD）**
  ...
  - **产物**：`claude_code/core/agent.py`：维护 messages、调用 LLM、处理工具结果、输出
...
- [ ] **T070 权限规则模型（deny/allow/ask）（TDD）**
  - **依赖**：T020, T013
  - **产物**：`claude_code/security/permissions.py`：文件/命令/网络访问规则
  - **改动点**：`claude_code/security/permissions.py`，`tests/test_permissions.py`
...
- [ ] **T071 文件读/写与命令执行的审批接入（TDD）**
  ...
  - **改动点**：`claude_code/security/permissions.py` + 调用方（`core/executor.py`/`core/context.py`），`tests/test_permissions.py`, `tests/test_executor.py`, `tests/test_context.py`
...
- [ ] **T072 Checkpoint 管理器（快照/回滚元数据）（TDD）**
  - **依赖**：T070
  - **产物**：`claude_code/security/checkpoint.py`：会话级 checkpoint 数据结构与持久化方案（MVP 简化）
  - **改动点**：`claude_code/security/checkpoint.py`，`tests/test_checkpoint.py`
...
- [ ] **T073 `/rewind` 命令：回滚 code / conversation / both（TDD）**
  ...
  - **改动点**：`claude_code/cli/commands.py`（或等价命令模块），`tests/test_rewind.py`
...
- [ ] **T080 Slash Commands 发现与解析（TDD）**
  - **依赖**：T021
  - **产物**：`claude_code/interaction/commands.py` 扫描 `.claude/commands/` 与 `~/.claude/commands/`
  - **改动点**：`claude_code/interaction/commands.py`，`tests/test_commands.py`
...
- [ ] **T082 Hooks 系统（事件定义 + 分发 + 脚本执行）（TDD）**
  - **依赖**：T021, T070
  - **产物**：`claude_code/interaction/hooks.py`：SessionStart/PreToolUse/PostToolUse 等最小闭环
  - **改动点**：`claude_code/interaction/hooks.py`，`tests/test_hooks.py`
...
- [ ] **T110 SDD 任务模型与解析器骨架（TDD）**
  ...
  - **改动点**：`claude_code/core/sdd.py`，`tests/test_sdd.py`
...
- [ ] **T111 从 `spec.md` 生成 `plan.md` 的最小工作流（TDD）**
  ...
  - **改动点**：`claude_code/core/sdd.py` + CLI 命令模块，`tests/test_sdd_flow.py`
...
- [ ] **T114 任务执行器（顺序执行 + 最小并行标记识别）（TDD）**
  ...
  - **改动点**：`claude_code/core/sdd.py`（或独立 executor），`tests/test_task_runner.py`
