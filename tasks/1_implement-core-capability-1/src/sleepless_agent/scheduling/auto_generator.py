...
class AutoTaskGenerator:
    ...
    def _determine_generation_mode(self, task_count: int) -> str:
        ...
        if task_count >= 5:
            ...
            # When queue is empty, use refine_focused to improve existing work
            # (new_friendly is disabled with weight=0)
            return "refine_focused"
    ...
    async def _run_prompt(self, prompt_config: AutoTaskPromptConfig, context: dict) -> Optional[str]:
        ...
        # Detect and reject non-task responses (AI explaining it can't generate a task)
        if self._is_rejection_response(full_text):
            logger.debug("autogen.prompt.rejected", prompt=prompt_config.name, reason="AI returned non-task response")
            return None
    ...
    def _is_rejection_response(text: str) -> bool:
        """Detect if the AI response is a rejection (no valid task to generate).

        Returns True if the response indicates the AI couldn't generate a valid task.
        """
        import re
        ...
        # Patterns that indicate a rejection/non-task response
        rejection_patterns = [
            r"cannot generate",
            r"no refine task can be generated",
            r"no existing work",
            r"nothing to refine",
            r"no tasks? available",
            r"unable to generate",
            r"i cannot",
            r"there are no (existing|active|pending) tasks?",
            r"workspace appears (empty|not initialized)",
            r"no (recent|previous) work",
            r"would you like me to",
            r"please provide",
            r"could you please",
            # Chinese patterns for GLM Coding
            r"没有(现有)?任务",
            r"无法生成",
            r"没有工作可做",
            r"工作区为空",
        ...
        # Also reject if the response is too long (likely an explanation, not a task)
        # Valid task descriptions should be 1-2 sentences (< 300 chars)
        if len(text) > 500:
            # But allow if it has proper task markers
            if not re.search(r'\[\s*(refine|new)', text_lower, re.IGNORECASE):
                return True
